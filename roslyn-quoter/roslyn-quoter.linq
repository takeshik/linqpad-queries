...
/*
 * RoslynQuoter on LINQPad, written by Takeshi KIRIYA (aka takeshik) <https://takeshik.org>.
 * Licensed under the Apache License, version 2.0.
 *
 *   This is a derived work of RoslynQuoter <https://github.com/KirillOsenkov/RoslynQuoter>.
...
void OpenQuery()
{
    var sfImport = string.IsNullOrEmpty(_syntaxFactoryId.Text)
        ? "static Microsoft.CodeAnalysis.CSharp.SyntaxFactory"
        : $"{_syntaxFactoryId.Text} = Microsoft.CodeAnalysis.CSharp.SyntaxFactory";
    var linqpadQuery = $@"<Query Kind=""Expression"">
  <NuGetReference>Microsoft.CodeAnalysis.Compilers</NuGetReference>
  <NuGetReference>Microsoft.CodeAnalysis.CSharp</NuGetReference>
  <Namespace>{sfImport}</Namespace>
  <Namespace>Microsoft.CodeAnalysis.CSharp.Syntax</Namespace>
  <Namespace>Microsoft.CodeAnalysis.CSharp</Namespace>
  <Namespace>Microsoft.CodeAnalysis</Namespace>
</Query>
...
{_code}";
    ...
    File.WriteAllText(tmpPath, linqpadQuery);
...
public class Quoter
{
    ...
    public string SyntaxFactorySymbol { get; set; } = "SyntaxFactory";
    ...
    private ApiCall QuoteNode(SyntaxNode node, string name)
    {
        ...
        if (!string.IsNullOrEmpty(SyntaxFactorySymbol))
        {
            factoryMethodName = SyntaxFactorySymbol + "." + factoryMethodName;
        }
    ...
    private object InterpretMemberCall(MemberCall memberCall)
    {
        ...
        if (!string.IsNullOrEmpty(SyntaxFactorySymbol) && name.StartsWith($"{SyntaxFactorySymbol}."))
        {
            name = name.Substring($"{SyntaxFactorySymbol}.".Length);
        }
